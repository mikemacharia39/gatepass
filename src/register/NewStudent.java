...
public class NewStudent extends Application{
	...
	Scene scene;
	Label lnametop, lpic;
	Image img= new Image(NewStudent.class.getResourceAsStream("/pic/addp.png"));
	...
	DatePicker txtdob;
	Label lfname, llname, ladm, lphone, lid, laddress, ltown, ldob, 
	...
	Image imagepic= new Image(NewStudent.class.getResourceAsStream("/pic/anon.png"));
	ImageView iv2= new ImageView(imagepic);
	
	Image imgfinger= new Image(NewStudent.class.getResourceAsStream("/pic/searchfg.png"));
	...
	public void start(Stage ns) throws Exception {
		...
		laypers2.getChildren().addAll(spaceadm, ladm, txtadm, lid, txtid, ldob, txtdob);
		...
		window.setTitle("Enrol new Student");
	        	...
	        	fileChooser.getExtensionFilters().addAll(
	                 ...
	                 try {
	                	// file=fileChooser.get
	                     ...
	                     Logger.getLogger(NewLecturer.class.getName()).log(Level.SEVERE, null, ex);
	                 }
	        }
		});
	...
	public void setAddClicked(){
	      ...
	      LocalDate dob1= txtdob.getValue();
	      ...
	      {
	    	  ErrorMessage.display("Alert", "School id required, please fill to proceed");
	    	  txtadm.requestFocus();
	      }
         ...
         try {
            ...
            DBConnect.connect();
        	 fis = new FileInputStream(file);
		         pstmt = DBConnect.conn.prepareStatement("INSERT INTO students(prefix, first_name, last_name, s_id,	n_id, phone, address, "
		         		+ "town, dob, thumb_id, student, picture) "
		         		+ "VALUES(?,?,?,?,?,?,?,?,?,?,?,?)");
		         ...
		         pstmt.setString(11, "student");
		         pstmt.setBinaryStream(12,(InputStream)fis, (int)file.length());
