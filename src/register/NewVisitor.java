...
public class NewVisitor extends Application{

	Stage window;
	Scene scene;
	Label lnametop, lpic;
	Image img= new Image(NewVisitor.class.getResourceAsStream("/pic/addp.png"));
	ImageView iv = new ImageView(img);
	Button btnadd, btnclose, btnenrolfinger, btnphoto;
	ComboBox<String> cbCourse;
	TextField txtfname, txtlname, txtphone, txtid, txtaddress, txttown, txtenrolfinger;
	...
	Image imagepic= new Image(NewVisitor.class.getResourceAsStream("/pic/anon.png"));
	ImageView iv2= new ImageView(imagepic);

	Image imgfinger= new Image(NewVisitor.class.getResourceAsStream("/pic/searchfg.png"));
	...
	public void start(Stage ns) throws Exception {
		...
		scene.getStylesheets().add(NewVisitor.class.getResource("newPeople.css").toExternalForm());
	...
	public void setAddClicked(){
		...
		{	
			...
			try {
				...
				pstmt = DBConnect.conn.prepareStatement("INSERT INTO visitors(prefix, first_name, last_name, s_id, n_id, "
				+ " department, phone, address, "
				+" town, thumb_id, visitor, picture) "
				+"VALUES(?,?,?,?,?,?,?,?,?,?,?,?)");
	...
	private boolean initialize(String port) {
		...
		try {
			...
			Logger.getLogger(NewVisitor.class.getName())
