...
public class NewLecturer extends Application{

	Stage window;
	Scene scene;
	Label lnametop, lpic;
	Image img= new Image(NewLecturer.class.getResourceAsStream("/pic/addp.png"));
	...
	Image imagepic= new Image(NewLecturer.class.getResourceAsStream("/pic/anon.png"));
	ImageView iv2= new ImageView(imagepic);
	
	Image imgfinger= new Image(NewLecturer.class.getResourceAsStream("/pic/searchfg.png"));
	...
	public void start(Stage ns) throws Exception {
		...
		//btnenrol it has action listener
		...
		scene.getStylesheets().add(NewLecturer.class.getResource("newPeople.css").toExternalForm());
		window.setScene(scene);
		window.setResizable(false);
		window.setTitle("Enrol new Lecturer");
		window.show();
		initialize("COM25");
           ...
           try {
              ...
              DBConnect.connect();
          	 fis = new FileInputStream(file);
		         pstmt = DBConnect.conn.prepareStatement("INSERT INTO lecturers(prefix, first_name, last_name, s_id, n_id, department, phone,"
		         		+ "address, town, thumb_id, lecturer, picture) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)");
